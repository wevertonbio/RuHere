...
* Updated the documentation by replacing `\dontrun{}` with `\donttest{}` in all functions involving data downloads (such as `wcvp_here()`, `prepare_gbif_download()`, and `request_gbif()`) or interactive visualizations (`map_here()`). The use of `\dontrun{}` was reserved for credential-setting functions, such as `set_gbif_credentials()` and `set_specieslink_credentials()`, as these fall under the policy exception for requiring private API keys.
