...
- correct_country:
  ...
  each record falls within the country specified in the metadata.
  Default is 'correct_country'. See details.
...
`lat` columns corrected, and an additional column (`country_issues`)
...
- **swapped**: longitude and latitude are transposed (i.e., each appears
  ...
  assigned country and could not be corrected.
...
`correct_country` argument) with logical values indicating which records
...
For each test, country borders are buffered by `distance` km to account
