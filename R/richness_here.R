...
#' final output. If the CRS differs from the output, it will be
#' automatically reprojected. Default is `NULL`.
...
richness_here <- function(occ, species = "species", long = "decimalLongitude",
                          ...
                          res = NULL, mask = NULL, summary = "records",
                          field = NULL, fun = mean, verbose = TRUE) {

  if (!inherits(occ, "data.frame")) stop("`occ` must be a data.frame.", call. = FALSE)
  if (!is.null(species) && !inherits(species, "character")) stop("`species` must be a character vector.", call. = FALSE)
  if (!inherits(long, "character")) stop("`long` must be a character vector.", call. = FALSE)
  if (!inherits(lat, "character")) stop("`lat` must be a character vector.", call. = FALSE)
  if (!inherits(summary, "character") || !summary %in% c("records", "species")) stop("`summary` must be either 'records' or 'species'.", call. = FALSE)
  if (!inherits(fun, "function")) stop("`fun` must be a function.", call. = FALSE)
  if (!is.null(res) && !inherits(res, c("numeric", "integer"))) stop("`res` must be a numeric value.", call. = FALSE)
  if (!is.null(raster_base) && !inherits(raster_base, "SpatRaster")) stop("`raster_base` must be a SpatRaster object.", call. = FALSE)
  if (!is.null(mask) && !inherits(mask, c("SpatRaster", "SpatVector"))) stop("`mask` must be a SpatRaster or SpatVector.", call. = FALSE)
  ...
  if (!all(required_cols %in% names(occ))) {
    ...
    stop("The following columns are missing in `occ`: ", paste(missing, collapse = ", "), call. = FALSE)
  }
  ...
  if (!is.null(raster_base)) {
    target_crs <- terra::crs(raster_base)
  } else if (!is.null(mask)) {
    target_crs <- terra::crs(mask)
  }

  if (!is.null(raster_base)) {
    r_template <- raster_base
  } else {
    if (is.null(res)) stop("You must provide either `raster_base` or a `res` value.", call. = FALSE)
    ext_pts <- terra::ext(min(occ[[long]]), max(occ[[long]]),
                          min(occ[[lat]]), max(occ[[lat]]))
    r_template <- terra::rast(ext_pts, res = res, crs = target_crs)
  }
  ...
  if (summary == "records") {
    ...
    if (!is.null(field)) {
      if (!is.null(names(field))) {
        ...
        if (!all(spp_in_df %in% names(field))) {
          if(verbose) warning("Some species in `occ` do not have a match in the `field` names. Values will be set to NA.")
        }
        ...
        if (!all(field %in% names(occ_internal))) {
          ...
          stop("The following `field` columns are missing in `occ`: ", paste(missing_f, collapse = ", "), call. = FALSE)
        }
        trait_cols <- field
      }
    }
    ...
    temp_data <- data.frame(cell_id = cell_ids, species = occ_internal[[species]])
    if (!is.null(trait_cols)) temp_data <- cbind(temp_data, occ_internal[, trait_cols, drop = FALSE])

    occ_unique <- unique(temp_data)
    pts_unique <- terra::vect(terra::xyFromCell(r_template, occ_unique$cell_id), crs = target_crs)
    ...
    if (is.null(field)) {
      ...
      r_final <- terra::rasterize(pts_unique, r_template, field = trait_cols, fun = fun, na.rm = TRUE)
  ...
  if (!is.null(mask)) {
    ...
    r_final <- terra::mask(r_final, mask)
  }
